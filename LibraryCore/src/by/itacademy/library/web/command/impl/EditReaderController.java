...
public class EditReaderController implements Controller {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        if (req.getMethod().equals("GET")) {
            ...
            if (req.getParameter("surname").matches("^[À-ß¨]([a-ÿ¸]){0,29}$") || req.getParameter("surname").length() == 0) {
                newReader.setSurname(req.getParameter("surname"));
            } else {
                validData = false;
            }
            if (req.getParameter("name").matches("^[À-ß¨]([a-ÿ¸]){0,29}$") || req.getParameter("name").length() == 0) {
                newReader.setName(req.getParameter("name"));
            } else {
                validData = false;
            }
            if (req.getParameter("secondname").matches("^[À-ß¨]([a-ÿ¸]){0,29}$") || req.getParameter("secondname").length() == 0) {
